...
public interface IInfoMapper {
    ...
    final String GET_ACCOUNT = "SELECT account_uid, name, email, password FROM account WHERE account_uid = 1;";
    final String UPDATE_ACCOUNT = "UPDATE account SET name = '${name}', email = '${email}', password = '${password}' WHERE account_uid = '${id}'";
    final String GET_ALL = "SELECT account_uid, name, email, password FROM account";
    //rewrite
...
//      @Result(property="rooms", column="id", javaType=Room.class, many="getRoomsForAccount"),
    ...
    public Account getAccountById(int account_uid) throws Exception;
